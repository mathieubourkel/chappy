...
export default function MagicButton({type, handleClick, wrap, value, reverse}:Props) {
    ...
    const renderButton = () => (
        <Button className={`${props.className} ${props.color}`} type={props.type} variant={props.variant} size={"sm"} onClick={props.onClick}>
            {renderIcon()}
            {renderText()}
        </Button>
    )
    ...
    if (wrap) return (
        <Tooltip
            content={props.value}
            className={`${props.tooltip} ${props.color} px-4`}
            animate={{
                mount: { scale: 1, y: 0 },
                unmount: { scale: 0, y: 25 },
            }}
        >
            {reverse ? renderReverseButton() : renderButton()}
        </Tooltip>
    ) 
